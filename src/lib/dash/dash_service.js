...
Networks.add(constants.DASH_MAINNET);
...
class DashService extends Service {
  constructor({
    ...
    withdrawal,
    dashRpc: api,
    dashInterpreter: interpreter,
    dashMaximumInput,
    dashMaximumFee,
    dashMaximumFeePerByte,
    dashAverageFeeBlocks,
  }) {
    const error = {
    ...
    this.maximumFee = Number(dashMaximumFee);
    this.maximumFeePerByte = parseInt(dashMaximumFeePerByte, 10);
    this.averageFeeBlocks = parseInt(dashAverageFeeBlocks, 10);
  }
