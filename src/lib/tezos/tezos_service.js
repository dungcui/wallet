...
class TezosService extends Service {
  constructor({
    db,
    block,
    token,
    ...
    withdrawal,
    tezosRpc: api,
    tezosInterpreter: interpreter,
    tezosMaximumInput,
    tezosMaximumFee,
    tezosMaximumFeePerByte,
    tezosAverageFeeBlocks,
  }) {
    const error = {
    ...
    this.maximumFee = Number(tezosMaximumFee);
    this.maximumFeePerByte = parseInt(tezosMaximumFeePerByte, 10);
    this.averageFeeBlocks = parseInt(tezosAverageFeeBlocks, 10);
  }
  ...
  async getMeta(wallet, transactions ,isColdWallet = false) {
    ...
    const total = transactions.reduce((acc, t) => acc.add(t.amount), new Decimal(0))
    .add(this.maximumFee);
  ...
  async validateAddress(req) {
    const { hash } = req;
    try {
      const address = Address.fromString(hash, Networks.DASH_MAINNET);
  ...
  { 
    ...
    try {
      console.log('DASH_Service.getTotalBalance')
      const wallet = await this.wallets.find(walletId);
      const balance = await this.computeTotalBalances(wallet, isColdWallet);
      console.log('balance:',balance);
      totalBalance = balance.DASH.amount;
    }
    catch (err) {
      console.log('DASH.getTotalBalance.err:',err);
      totalBalance = "0";
    }
    console.log('DASH.totalBalance:',totalBalance);
