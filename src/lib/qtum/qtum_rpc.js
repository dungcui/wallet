...
class QtumRpc {
  constructor({ qtumNodeUrl }) {
    this.nodeUrl = qtumNodeUrl;
    this.sleepTime = 10;
    this.MAX_ATTEMPT = 20;
    if (!this.nodeUrl) {
      throw Error('Please provide QTUM_NODE_URL');
    }
  ...
  async getRawTx(txHash, verbose = 1, attempt = 0) {
    try {
      ...
      await Promise.delay(1000 * this.sleepTime);
      return await this.getRawTx(txHash, verbose = 1, attempt + 1);
    }
