...
// const debug = require('debug')('wallet:bsv_monitor');
const constants = require('./bsv_constants');
const { rangeToArray } = require('../../utils');
const Decimal = require('decimal.js');

class BsvMonitor extends Monitor {
  constructor({
    bsvSleepTime, bsvStartBlockHeight,bsvMinimumConfirmation,bsvRpc,
    db, block, token,wallet,  bsvInterpreter, funding, withdrawal,failedApi, limit, address,
    bsvMaximumInput,
  }) {
    super({
      db,
      ...
      token,
      wallet,
      interpreter:bsvInterpreter,
      funding,
      withdrawal,
      failedApi,
      limit,
      address,
      startBlockHeight:bsvStartBlockHeight,
      minimumConfirmation:bsvMinimumConfirmation,
      sleepTime:bsvSleepTime,
    });
    // For register currency mapping
    this.maximumInput = Number(bsvMaximumInput);
  }
  ...
  //           }
  //           // Convert from satoshi tobsv
  //           balancesHash[address][txHash] = new Decimal(balancesHash[address][txHash])
  //             .add(new Decimal(o.value).div(bsv_TO_SATOSHI))
  //             .toString();
  //         }
  //       }
  //     });
  ...
  //   }
  //   const reward =bsvMonitor.getBlockReward(height);
