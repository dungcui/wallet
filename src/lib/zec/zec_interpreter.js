...
class ZecInterpreter {
  constructor({ address, funding, zecRpc }) {
  ...
  async derive(wallet, path) {
    ...
    const xpub = new HDPublicKey(wallet.xpubs);
    const address = new Address(xpub.derive(`m/${path}`).publicKey, Networks.livenet).toString();
    return { address };
  }
