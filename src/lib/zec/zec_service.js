...
const { Address, HDPublicKey, Networks } = require('zcash-bitcore-lib');
const utils = require('../../utils');
const constants = require('./zec_constants');
const Service = require('../service');
const debug = require('debug');


// Networks.add(constants.zec_MAINNET);

class ZecService extends Service {
  constructor({
    db,
    block,
    token,
    ...
    withdrawal,
    zecRpc: api,
    zecInterpreter: interpreter,
    zecMaximumInput,
    zecMaximumFee,
    zecMaximumFeePerByte,
    zecAverageFeeBlocks,
  }) {
    const error = {
    ...
    this.maximumInput = Number(zecMaximumInput);
    this.maximumFee = Number(zecMaximumFee);
    this.maximumFeePerByte = parseInt(zecMaximumFeePerByte, 10);
    this.averageFeeBlocks = parseInt(zecAverageFeeBlocks, 10);
  ...
  async validateAddress(req) {
    const { hash } = req;
    try {
      const address = Address.fromString(hash, Networks.zec_MAINNET);
  ...
  { 
    console.log('ZEC_Service.getTotalBalance')
    let totalBalance = "";
    ...
    try {
      ...
      totalBalance = balance.ZEC.amount;
    } 
    catch (err) {
      console.log('ZEC.getTotalBalance.err:',err);
      totalBalance = "0";
    }
    console.log('ZEC.totalBalance:',totalBalance);
    return {currency, totalBalance }
  }
}

module.exports = ZecService;
