...
const debug = require("debug")("wallet:etc_service");
...
class EtcService extends Service {
  constructor({
    db,
    block,
    token,
  ...
  async bundleMoveFund(req) {
    ...
    const transactions = await Promise.map(unspentMoveFunds, async funding => {
      ...
      return {
        ...
        grossAmount: new Decimal(funding.amount).div(1000000000).toFixed(),
        nonce,
  ...
  async getTotalBalance(currency, walletId, isColdWallet = true) {
    ...
    try {
      ...
      console.log("ETC.getTotalBalance.err:", err);
      totalBalance = "0";
    }
    console.log("ETC.totalBalance:", totalBalance);
