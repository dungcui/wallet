...
const transactionCreator = require('./eos_transaction_creator')
const SPACE = ' ';

class EosInterpreter {
  constructor({ db, address, eosApi ,eosTokenAccount , eosPublicHotKey,eosPublicColdKey}) {
    this.db = db;
    this.api = eosApi;
    this.addresses = address;
    this.eosTokenAccount = eosTokenAccount;
    this.eosTokenAccount = eosTokenAccount;
    this.eosPublicHotKey = eosPublicHotKey;
    this.eosPublicColdKey = eosPublicColdKey;
  }
  ...
  async getMeta(wallet, transactions, isColdWallet = true) {
    ...
    await Promise.each(transactions, async (inputTx) => {
      ...
      if(!isColdWallet)
      {
        requiredKeys [0] = this.eosPublicHotKey;
      } else 
      {
        requiredKeys [0] = this.eosPublicColdKey;
      }
  ...
  async parseTransaction(raw, blockHeight, trx) {
    console.log('*---- EOS_interpreter.parseTransaction ----*')
    console.log('raw:',raw)
