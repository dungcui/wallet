...
class LtcService extends Service {
  constructor({
    db,
    block,
    token,
    ...
    withdrawal,
    ltcRpc: api,
    ltcInterpreter: interpreter,
    ltcMaximumInput,
    ltcMaximumFee,
    ltcMaximumFeePerByte,
    ltcAverageFeeBlocks,
  }) {
    const error = {
    ...
    this.maximumFee = Number(ltcMaximumFee);
    this.maximumFeePerByte = parseInt(ltcMaximumFeePerByte, 10);
    this.averageFeeBlocks = parseInt(ltcAverageFeeBlocks, 10);

    // this.debug("ssss ",constants.LTC_MAINNET);
  }
  ...
  async validateAddress(req) {
    const { hash } = req;
    try {
      const address = Address.fromString(hash, Networks.LTC_MAINNET);
  ...
  { 
    ...
    try {
      console.log('LTC_Service.getTotalBalance')
      const wallet = await this.wallets.find(walletId);
      const balance = await this.computeTotalBalances(wallet, isColdWallet);
      console.log('balance:',balance);
      totalBalance = balance.LTC.amount;
    }
    catch (err){
      console.log('LTC.getTotalBalance.err:',err);
      totalBalance = "0";
    }
    console.log('LTC.totalBalance:',totalBalance);
