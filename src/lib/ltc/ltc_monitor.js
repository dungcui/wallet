...
class LtcMonitor extends Monitor {
  constructor({
    ltcMaximumFee,ltcMoveFundSleepTime,ltcSleepTime, ltcStartBlockHeight, ltcMinimumConfirmation, ltcRpc,
    db, block, token,wallet , ltcInterpreter, funding, withdrawal,failedApi, limit, moveFund, address,
    ltcMaximumInput,
  }) {
    super({
      db,
      ...
      moveFund,
      address,
      startBlockHeight: ltcStartBlockHeight,
      minimumConfirmation: ltcMinimumConfirmation,
      sleepTime: ltcSleepTime,
      moveFundSleepTime: ltcMoveFundSleepTime,

    });
    // For register currency mapping
    this.maximumInput = Number(ltcMaximumInput);
    this.ltcMaximumFee = Number(ltcMaximumFee);
  }
