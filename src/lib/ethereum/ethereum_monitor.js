...
class EthereumMonitor extends Monitor {
  constructor({
    ethereumSleepTime,
    ethereumStartBlockHeight,
    ethereumMinimumConfirmation,
    ethereumApi,
    db,
    block,
    token,
    ...
    // this.nodeUrl = ethereumNodeUrl || 'http://95.216.227.169:8545';
    ...
    super({
      ...
      token,
      wallet: wallet,
      interpreter: ethereumInterpreter,
      funding,
      withdrawal,
      failedApi,
      limit,
      address,
      startBlockHeight: ethereumStartBlockHeight,
      minimumConfirmation: ethereumMinimumConfirmation,
      sleepTime: ethereumSleepTime
    });
  ...
  validateBlock(block, fromHeight, toHeight) {
    console.log("*---- Monitor.validateBlock ----*");
    if (block) console.log("block :", block.height);
  ...
  //           funding.amount - Math.round(this.gasPrice * this.gasLimit) - 1000000
  //         )
  //           .div(1000000000)
  //           .toFixed(),
  //         nonce,
