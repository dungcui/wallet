...
class BchService extends Service {
  constructor({
    db,
    block,
    token,
    ...
    withdrawal,
    bchRpc: api,
    bchInterpreter: interpreter,
    bchMaximumInput,
    bchMaximumFee,
    bchMaximumFeePerByte,
    bchAverageFeeBlocks,
  }) {
    const error = {
    ...
    this.maximumFee = Number(bchMaximumFee);
    this.maximumFeePerByte = parseInt(bchMaximumFeePerByte, 10);
    this.averageFeeBlocks = parseInt(bchAverageFeeBlocks, 10);
  ...
  { 
    ...
    try {
      ...
      totalBalance = balance.BCHABC.amount;
    }
    catch (err)
    {
