...
class BchMonitor extends Monitor {
  constructor({
    bchSleepTime, bchStartBlockHeight, bchMinimumConfirmation, bchRpc,
    db, block, token, bchInterpreter, funding, withdrawal, failedApi, limit, address,
    bchMaximumInput,
  }) {
    super({
      db,
      ...
      block,
      token,
      interpreter: bchInterpreter,
      funding,
      withdrawal,
      failedApi,
      limit,
      address,
      startBlockHeight: bchStartBlockHeight,
      minimumConfirmation: bchMinimumConfirmation,
      sleepTime: bchSleepTime,
    });
    // For register currency mapping
    this.maximumInput = Number(bchMaximumInput);
  }
