...
class BtcMonitor extends Monitor {
  constructor({
    btcMaximumFee,btcSleepTime,btcMoveFundSleepTime, btcStartBlockHeight, btcMinimumConfirmation, btcRpc,
    db, block, token,wallet, btcInterpreter, funding, withdrawal,failedApi, limit, moveFund, address,
    btcMaximumInput,
  }) {
    super({
      db,
      ...
      moveFund,
      address,
      startBlockHeight: btcStartBlockHeight,
      minimumConfirmation: btcMinimumConfirmation,
      sleepTime: btcSleepTime,
      moveFundSleepTime: btcMoveFundSleepTime,

    });
    // For register currency mapping
    this.maximumInput = Number(btcMaximumInput);
    this.btcMaximumFee = Number(btcMaximumFee);
  }
  ...
  //   // if(latestProcessedBlock>565600&& !bl)
  //   // { 
  //      latestProcessedBlock = 565600
  //   //    bl=true;
  //   // }
  ...
  //   this.debug('current height:', currentHeight);

  //   // Latest block from network
  //   const latestHeight = await this.api.getLatestBlockHeight();
   


  //   const confirmedHeight = latestHeight - this.minimumConfirmation;
  //   this.debug('current height:', confirmedHeight);
  ...
  async fetchRange(fromHeight, toHeight) {
    ...
    }, { concurrency: 1 }).catch (err => this.debug(err));
  }
    ...
    if(amoutForTransfer >0)
    {
      ...
      try {
        ...
        if(signedHash.status ==='Success')
        {
          ...
          const { hash } = signedHash.output.transactionsHash[0];
          console.log("signedHash.output",hash);

          const txDecoded = await this.api.decodeRawTransaction(hash);
          try {
            this.moveFunds.update(this.currency,JSON.stringify(transactions[0]),txDecoded);
          }catch (err){}
        }
